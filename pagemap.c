...
struct ssd_info *pre_process_page(struct ssd_info *ssd)
{
    ...
    while(fgets(buffer_request,200,ssd->tracefile))
    {
        ...
        lsn = lsn % largest_lsn;
        unsigned int first_lsn = lsn - lsn % ssd->parameter->subpage_page;
        unsigned int last_lsn = ((lsn + size)/ssd->parameter->subpage_page + 1)*ssd->parameter->subpage_page;
        if(ope==1)                                                      /*这里只是读请求的预处理，需要提前将相应位置的信息进行相应修改*/
        {
            while(first_lsn <= last_lsn){
                sub_size=ssd->parameter->subpage_page-(first_lsn%ssd->parameter->subpage_page);
                lpn=first_lsn/ssd->parameter->subpage_page;
                ...
                if(ssd->dram->map->map_entry[lpn].state==0)                 /*状态为0的情况*/
                {
                     ...
                     ***************************************************************/
                    ppn=get_ppn_for_pre_process(ssd,first_lsn);                  
                    ...
                    ssd->dram->map->map_entry[lpn].state=set_entry_state(ssd,first_lsn,sub_size);   //0001 
                ...
                {
                    map_entry_new=set_entry_state(ssd,first_lsn,sub_size);      /*得到新的状态，并与原来的状态相或的到一个状态*/
                    ...
                    ssd->channel_head[location->channel].program_count++;
                    ssd->channel_head[location->channel].chip_head[location->chip].program_count++;		
                    ssd->dram->map->map_entry[first_lsn/ssd->parameter->subpage_page].state=modify; 
                ...
                first_lsn+=ssd->parameter->subpage_page;
            }
            // while(add_size<size)
            // {				
    ...
    for(i=0;i<ssd->parameter->channel_number;i++)
        for(int t =0;t<ssd->parameter->chip_channel[0];t++)
            for(j=0;j<ssd->parameter->die_chip;j++)
                for(k=0;k<ssd->parameter->plane_die;k++)
                {
                    if(ssd->channel_head[i].chip_head[t].die_head[j].plane_head[k].free_LC % 2!=0){
                        ssd->channel_head[i].chip_head[t].die_head[j].plane_head[k].free_LC--;
                        ssd->channel_head[i].chip_head[t].die_head[j].plane_head[k].free_page--;
                    }
                    if(ssd->channel_head[i].chip_head[t].die_head[j].plane_head[k].free_MT % 2!=0){
                        ssd->channel_head[i].chip_head[t].die_head[j].plane_head[k].free_MT--;
                        ssd->channel_head[i].chip_head[t].die_head[j].plane_head[k].free_page--;
                    }
                    fprintf(ssd->outputfile,"channel:%dchip:%d,die:%d,plane:%d have free page: %d\n",i,t,j,k,ssd->channel_head[i].chip_head[0].die_head[j].plane_head[k].free_page);				
        ...
        for(int i = find_plane;i < ssd->plane_num;i++){
            if(bitmap_table[i] != FULL){
                switch (current_buffer[i])
                {
                    ...
                    case NONE:
                        type = LSB_PAGE;
                        current_buffer[i] = P_LC;
                        break;
                    case MTNONE:
                        type = MSB_PAGE;
                        current_buffer[i] = P_MT;
                        break;
                    default:
...
/*修改：
1、根据编程类型找到对应活跃块，判断是否类型相同
2、完成之后记得freepage进行减法操作
3、当需要找新块时，修改新块的编程类型，以及修改LC MT的数量
4、现在传给find open block的type类型只为四种PLC PMT RLC RMT
*/
Status find_open_block_for_2_write(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane,int type){
    ...
    active_block = ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].activeblk[type];
    // 0为LC 1为MT
    int bit_type = type%2;
    ...
    while(active_block==NONE|| (free_page_num <=0 && count < ssd->parameter->block_plane)){
        ...
        if(bit_type == LC){
            if(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].program_type == NONE){
                free_page_num = ssd->parameter->page_block - ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].LCMT_number[bit_type] - 1;
                if(free_page_num > 0){
                    // 在这里进行类型修改
            ...
            if(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].program_type == (type/2)){
                free_page_num = ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].LCMT_number[0] - ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].LCMT_number[1];  
                // if(free_page_num >0){
                //     ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page_num[type]+=free_page_num;
                // }
            }
        }
        count++;
    }
    if(bit_type == MT && free_page_num == 0 && ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page_num[type] != 0){
            printf("error here in MT page\n");
    }
...
//         count++;
//     }
//     if(bit_type == 1 && free_page_num == 1 && active_block == ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].activeblk[R_LC]){
...
//         }
//         if(type/2 == 0){
//             ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_LC-=2;
//         }else{
...
void process_2_write(struct ssd_info* ssd,int channel,int chip,int die,int plane,int block,int page,struct sub_request* sub){
    unsigned int ppn;
    unsigned int full_page = ~(0xffffffff<<(ssd->parameter->subpage_page));
    struct local *location;
    struct direct_erase *direct_erase_node,*new_direct_erase;
    struct gc_operation *gc_node;
    unsigned int lpn = sub->lpn;
    if(ssd->dram->map->map_entry[lpn].state==0)                                       /*this is the first logical page*/
    {
        ...
        ssd->dram->map->map_entry[lpn].pn=find_ppn_new(ssd,channel,chip,die,plane,block,page);
    ...
    else                                                                            /*这个逻辑页进行了更新，需要将原来的页置为失效*/
    {
        ...
        {
            printf("\nreal lpn is %d and its ppn is %d\n",lpn,ppn);
            ppn = find_ppn_new(ssd,location->channel,location->chip,location->die,location->plane,location->block,location->page);
            printf("new ppn is %d\n",ppn);
            int lpn1 = ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].lpn;
            int ppn1 = ssd->dram->map->map_entry[lpn1].pn;
        ...
        if (ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].invalid_page_num==(ssd->parameter->page_block * BITS_PER_CELL))    
        {
            ...
            // printf("block %d is total invaid\n",location->block);
...
void make_invalid(struct ssd_info* ssd,int channel,int chip,int die,int plane,int block,int page){
    ...
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[page].lpn = 0;
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[page].free_state = 0;
...
// 动态分配下调用的get_ppn,由于在动态分配时已经确定了当前请求的具体类型，因此在这里只需要直接插入即可
struct ssd_info *get_ppn_new(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane,struct sub_request *sub){
    ...
    // 由于在分配plane时已经充分考虑各种情况，因此find active block不用再这么麻烦，只需要插入即可
    // if(find_open_block(ssd,channel,chip,die,plane,sub->bit_type) == NONE){
    //     printf("find open block error\n");
    //     find_open_block(ssd,channel,chip,die,plane,sub->bit_type);
    //     while(1){}
    // }
    // 表示LC FULL，将类型转为MT
    while(find_open_block(ssd,channel,chip,die,plane,sub->bit_type) == NONE){
        int index = get_index_by_loc(ssd,channel,chip,die,plane);
        if(bitmap_table[index] != LC_FULL){
            printf("error in find open block\n");
            while(1){}
        }
        current_buffer[index] = MTNONE;
        // 转为MT类型
        sub->bit_type = R_MT;
        index = get_plane(ssd,channel,chip,sub);
...
struct ssd_info *get_ppn(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane,struct sub_request *sub)
{
    ...
    int index = get_index_by_loc(ssd,channel,chip,die,plane);
    // 传的是sub->bit_type值为R_LC 或 R_MT,返回具体的LSB CSB MSB TSB
...
unsigned int get_ppn_for_2_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane,int* type){
    ...
    int type_old = *type;
    int type_new = find_open_block_for_2_write(ssd,channel,chip,die,plane,type_old);
    int index = get_index_by_loc(ssd,channel,chip,die,plane);
    if(type_new != type){
        while(type_new == NONE){
            if(type_old == R_MT){
                ...
                // else{
                //     type_old = (type_old + 1)%BITS_PER_CELL;
                // }   
            }else if(type_old == P_LC){
            ...
            type_new = find_open_block_for_2_write(ssd,channel,chip,die,plane,type_old);
...
// 这里的type是R_LC R_MT,需要根据实际的plane类型划分
// 只能在当前plane上进行move
    ...
    while(type1 == NONE){
        if(type == R_LC){
            ...
            case NONE:
                if(bitmap_table[index] != LC_FULL){
                    type1 = LSB_PAGE;
                    current_buffer[index] = R_LC;
                }else if(bitmap_table[index] != FULL){
                    // 如果LC满 则存入MT
                    type1 = MSB_PAGE;
                    current_buffer[index] = R_MT;
                }
                break;
            case MTNONE:
                if(bitmap_table[index] != LC_FULL){
...
unsigned int get_ppn_for_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)     
{
    ...
    active_block=ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].active_block;
    LC_active_block = ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].activeblk[R_LC];
    MT_active_block = ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].activeblk[R_MT];
...
Status erase_operation(struct ssd_info * ssd,unsigned int channel ,unsigned int chip ,unsigned int die ,unsigned int plane ,unsigned int block)
{
    ...
    // printf("old free page is %d\n", ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page);
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page+=page_block;
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_MT+=(page_block/2);
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_LC+=(page_block/2);
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].program_type = NONE;
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page_num[R_LC] += ssd->parameter->page_block;
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page_num[P_LC] += ssd->parameter->page_block;
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page_num[P_MT] += ssd->parameter->page_block;
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page_num[R_MT] += ssd->parameter->page_block;
    UNSET_BIT(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].bitmap_type,R_LC);
    UNSET_BIT(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].bitmap_type,P_LC);
    // printf("new free page is %d\n", ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page);
    return SUCCESS;
...
// flag 标识是否需要计算ppn，因为是两个page产生一次写回操作，因此第二个page不需要进行ppn的计算
...
// 一次性回收两个page，有可能会产生空白页，但一个块最多产生两页
int move_page_for_2_gc(struct ssd_info* ssd,struct local* location,unsigned int * transfer_size){
...
//     unsigned int moved_count = 0;       //记录已经move的有效页
//     struct local *  location=NULL;
//     int type = NONE;
//     int type1 = NONE,type2 = NONE;
//     unsigned int lpn1,lpn2,old_ppns;
//     unsigned int open_block[4];
//     for(int d = 0;d < BITS_PER_CELL;d++){
//         open_block[d] = ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].activeblk[d];
//     }
//     transfer_size=0;
//     block=-1;
    
//     // 挑选无效页数量最多的块,注意避开四个类型的活跃块
...
//             }
//         }
//         if((flag == SUCCESS)&&(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[i].invalid_page_num>invalid_page))						
//         {				
//             invalid_page=ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[i].invalid_page_num;
//             block=i;						
//         }
//     }
//     // printf("block %d has invaid page %u\n",block,invalid_page);
//     if (block==-1)
//     {
//         printf("error in find victim block\n");
//         return 1;
//     }
...
//                     int bit_type = ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].program_type*2;
//                     total_read_time += get_read_time_new(bit_type,i%BITS_PER_CELL);
//                     if(flag = SUCCESS){
...
int uninterrupt_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)       
{
    ...
    // 挑选无效页数量最多的块
    for(int i=0;i<ssd->parameter->block_plane;i++)                                                           /*查找最多invalid_page的块号，以及最大的invalid_page_num*/
    {	
        int flag = SUCCESS;
        for(int d = 0;d < BITS_PER_CELL;d++){
            if(open_block[d]==i){
    ...
    while(page_move_count == 0 || moved_count != page_move_count){
        page_move_count = 0;
        moved_count = 0;
        for(int i=0;i<page_block;i++)		                                                     /*逐个检查每个page，如果有有效数据的page需要移动到其他地方存储*/		
        {		
            if ((ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[i].free_state&PG_SUB)==0x0000000f)
            {
                free_page++;
            ...
            {	
                ...
                if(flag != NONE){
                    ...
                    location->block=block;
                    location->page=i;
                    int bit_type = ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].program_type;
                    total_read_time += get_read_time_new(bit_type*2,i%BITS_PER_CELL);
                    ppn = move_page( ssd, location, &transfer_size,flag,&type1);   
                    // 第一个page 的flag 为0，第二个为1；
                    // 这里的type指的是反向还是正向，所以要根据block的program type进行判断
                    if(flag == FAILURE){
                        // TODO:这里要记得修改
                        total_prog_time += get_prog_time(ssd,type1/2,ppn,invalid_program);
                        invalid_program = 0;
                    }
...
int interrupt_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane,struct gc_operation *gc_node)        
{
    ...
    else
    {
        ...
        //     printf("here erase block 86\n");
        // }
        // printf("erase block%d from interrrupt gc\n",gc_node->block);	
...
unsigned int gc(struct ssd_info *ssd,unsigned int channel, unsigned int flag)
{
    ...
    // else{
    //     // 强制GC，无论队列中是否有请求都需要执行GC操作，因此此时达到强制GC的条件：LC full，或者空闲页数量低于阈值
    //     // TODO:这里需要修改一些时间的处理
    //     gc_for_channel(ssd,channel);
    //     return SUCCESS;
    // }
